...
// 1. 读取模板（sb_1.14_v4.json / sb_1.14_v6.json，配套 sing-box 1.14）
let config = JSON.parse($files[0]);

// 2. 插入自定义路由规则，默认插在 global 模式规则之后，便于覆盖默认分流。
if (rulesFile) {
  try {
    ...
    if (customRulesRaw) {
      ...
      const deduped = customRules.filter(rule => !existingRules.has(JSON.stringify(rule)));

      const insertIndex = config.route.rules.findIndex(rule => rule.clash_mode === "global");
      if (deduped.length > 0) {
        if (insertIndex >= 0) {
          config.route.rules.splice(insertIndex + 1, 0, ...deduped);
        } else {
          config.route.rules.unshift(...deduped);
        }
      }
    }
  } catch (error) {
...
config.outbounds.forEach(group => {
  ...
  switch (group.tag) {
    case "♻️ 中转分组":
      group.outbounds.push(DIRECT_TAG, ...softbankTags, ...azureTags);
      break;
    case "🛬 落地分组":
      group.outbounds.push(DIRECT_TAG, ...jmsTags, ...dmitTags);
      break;
    case "🚀 节点选择":
      group.outbounds.push(DIRECT_TAG, ...dmitTags, ...azureTags, ...softbankTags, ...jmsTags);
      break;
    case "🤖 AI":
      group.outbounds.push(DIRECT_TAG, ...dmitTags, ...azureTags, ...softbankTags, ...jmsTags);
      break;
    // 💳 PayPal / github / 🪟 microsoft@cn / cn 使用模板默认（节点选择 + direct），脚本不注入
  }

  group.outbounds = unique(group.outbounds).filter(tag => tag !== group.tag);

  if (group.type === "selector" && group.outbounds.length === 0) {
